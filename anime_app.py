...
# Preload models when the app starts
load_models()

# Create a Gradio Blocks app (more modern approach)
def create_blocks_app():
    ...
    """) as blocks:
        gr.Markdown("# AnimeGANv2 Portrait Converter")
        gr.Markdown("Convert your portrait photos to anime-style images using the AnimeGANv2 model.")
        
        with gr.Row():
            with gr.Column():
                input_image = gr.Image(type="pil", label="Upload Portrait Photo")
                style_version = gr.Radio(
                    choices=['version 1 (more stylized)', 'version 2 (more robust)'],
                    value='version 2 (more robust)',
                    label="Style Version"
                )
                convert_btn = gr.Button("Convert to Anime Style")
            
            with gr.Column():
                output_image = gr.Image(type="pil", label="Anime Style Result")
        
        convert_btn.click(
            fn=process_image,
            inputs=[input_image, style_version],
...
# Create the app
app = create_blocks_app()

if __name__ == "__main__":
    # Launch the app
    app.launch(server_name="0.0.0.0", server_port=5000, show_error=True)